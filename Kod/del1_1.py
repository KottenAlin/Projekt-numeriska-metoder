...
def g(d):
    return (d/(D_s*h_t)+d*R_fi/D_s+d*np.log(d/D_s)/k_w+R_fo+1/h_s)
...
def g_prime(d):
    return (1/(D_s*h_t)+R_fi/D_s+(np.log(d/D_s)+1)/k_w)
...
if __name__ == "__main__":
    #plot_function(f)
    solution, errors = newton_method(d_0, tolerance)
    ...
    d_range = (0, 2)
    plot_function(f, d_range)

    start_guess = 0.001
    solution, _ = newton_method(start_guess, tolerance)
    ...
    print(f"Optimized Solution using fsolve with start guess {start_guess}: {optimized_solution[0]}")
